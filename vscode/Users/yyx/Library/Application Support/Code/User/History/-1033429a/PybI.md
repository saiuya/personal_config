...
高消費電力：
複数のタッチポイントを同時に処理するため、エネルギー消費が増加します。
バッテリー寿命が短くなる可能性があり、モバイルデバイスでは不利になることがあります。
誤操作のリスク：
意図しないタッチを認識してしまい、誤った操作が行われる可能性があります。
特に画面が小さいデバイスでは、誤タッチが発生しやすいです。
複雑な開発とメンテナンス：
ソフトウェア開発において、複雑なジェスチャー認識を実装する必要があり、開発コストと時間が増加します。
センサーの精度維持やキャリブレーションなど、メンテナンス面での課題もあります。