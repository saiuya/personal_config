...
import com.saiuya.blog.entity.*;
import com.saiuya.blog.enums.CommentTypeEnum;
import com.saiuya.blog.mapper.ArticleMapper;
...
public class CommentService extends ServiceImpl<CommentMapper, Comment> {
    ...
    public PageResult<CommentResp> listCommentVO(CommentQuery commentQuery) {
        ...
        // 父评论id集合
        List<Integer> parentCommentIdList = commentRespList.stream().map(CommentResp::getId)
                .collect(Collectors.toList());
        // 分组查询每组父评论下的子评论前三条
        List<ReplyResp> replyRespList = commentMapper.selectReplyByParentIdList(parentCommentIdList);
        // 封装子评论点赞量
        replyRespList.forEach(
                item -> item.setLikeCount(Optional.ofNullable(likeCountMap.get(item.getId().toString())).orElse(0)));
        // 根据父评论id生成对应子评论的Map
    ...
    public List<ReplyResp> listReply(Integer commentId) {
        // 分页查询子评论
        List<ReplyResp> replyRespList = commentMapper.selectReplyByParentId(PageUtils.getLimit(), PageUtils.getSize(),
                commentId);
        // 子评论点赞Map
        Map<String, Integer> likeCountMap = redisService.getHashAll(RedisConstant.COMMENT_LIKE_COUNT);
    ...
    private void verifyComment(CommentReq comment) {
        if (comment.getCommentType().equals(CommentTypeEnum.ARTICLE.getType())) {
            Article article = articleMapper.selectOne(
                    new LambdaQueryWrapper<Article>().select(Article::getId).eq(Article::getId, comment.getTypeId()));
            Assert.notNull(article, "文章不存在");
        }
        if (comment.getCommentType().equals(CommentTypeEnum.TALK.getType())) {
            Talk talk = talkMapper
                    .selectOne(new LambdaQueryWrapper<Talk>().select(Talk::getId).eq(Talk::getId, comment.getTypeId()));
        ...
        Optional.ofNullable(comment.getParentId()).ifPresent(parentId -> {
            ...
            User toUser = userMapper
                    .selectOne(new LambdaQueryWrapper<User>().select(User::getId).eq(User::getId, comment.getToUid()));
    ...
    private void sendEmail(Comment comment, User toUser, String title, String fromNickname) {
        ...
        if (comment.getIsCheck().equals(CommonConstant.TRUE)) {
            ...
            // 发送HTML邮件
            // rabbitTemplate.convertAndSend(MqConstant.EMAIL_EXCHANGE,
            // MqConstant.EMAIL_HTML_KEY, mailDTO);
            ...
            // 发送普通邮件
            // rabbitTemplate.convertAndSend(MqConstant.EMAIL_EXCHANGE,
            // MqConstant.EMAIL_SIMPLE_KEY, mailDTO);
