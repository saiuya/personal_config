...
public class TouchMoving : MonoBehaviour
{
    ...
    public Vector3[] SampleUpperHalfCircle(Vector3 start, Vector3 end)
    {
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            Vector3 samplePoint;
            samplePoint.x = user.position.x + radius * Mathf.Cos(radian) * Mathf.Cos(0);
            samplePoint.y = user.position.y + radius * Mathf.Sin(radian);
            samplePoint.z = user.position.z + radius * Mathf.Cos(radian) * Mathf.Sin(0);
