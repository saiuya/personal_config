...
public class TouchMoving : MonoBehaviour
{
    ...
    // 移动时视角朝向
    private Transform look_ruiquanmen;
    private Transform look_fengshenmen;
    private Transform look_zhengdian;
    private Transform look_dongtai;
    private Transform look_wumen;
    private Transform look_taihedian;
    private Transform look_qianqinggong;
    private Transform look_shenwumen;
    ...
    void Start()
    {
        foreach (Transform child in anchorParent)
        {
            ...
            else if (child.name == "lookhead")
            {
                look_ruiquanmen = child.GetChild(0);
                look_fengshenmen = child.GetChild(1);
                look_zhengdian = child.GetChild(2);
                look_dongtai = child.GetChild(3);
                look_wumen = child.GetChild(4);
                look_taihedian = child.GetChild(5);
                look_qianqinggong = child.GetChild(6);
                look_shenwumen = child.GetChild(7);
            }
            else
            {
    ...
    public void GoTo_ruiquanmen()
    {
        user.DOPath(new Vector3[] {
            ...
            ruiquanmen_above.position,
            ruiquanmen.position,
        }, Speed, PathType.CatmullRom, PathMode.TopDown2D)
                    // .SetOptions(false, lockRotation: AxisConstraint.X)
                    // .SetOptions(false, lockRotation: AxisConstraint.Z)
                    .SetLookAt(look_ruiquanmen, forwardDirection: Vector3.right, up: Vector3.down)
