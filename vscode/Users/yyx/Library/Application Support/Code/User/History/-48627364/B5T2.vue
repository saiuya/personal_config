...
let label = ref({ startLabel: "那覇市", endLabel: "琉球大学" });
...
async function calculateAndDisplayRoute(
  ...
  directionsService
    .route({
      ...
      origin: label.value.startLabel,
      destination: label.value.endLabel,
  ...
  watch(label, (newLabel, oldLabel) => {
    directionsService
      .route({
        // origin: (document.getElementById("start") as HTMLSelectElement).value,
        // destination: (document.getElementById("end") as HTMLSelectElement).value,
        origin: newLabel.startLabel,
        destination: newLabel.endLabel,
        travelMode: TravelMode.DRIVING,
        transitOptions: {
          modes: [TransitMode.BUS],
        },
...
<template>
  ...
  <div id="floating-panel">
    <b>起点: </b>
    <input type="text" v-model="label.startLabel" placeholder="请输入出发地址" />
    &nbsp;
    <b>目的地: </b>
    <input type="text" v-model="label.endLabel" placeholder="请输入目的地" />
  </div>
