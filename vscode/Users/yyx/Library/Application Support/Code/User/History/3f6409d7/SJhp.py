...
class Transcribe:
    ...
    def translate(self, filepath):
        ...
        for dest in self.args.dest:
            srt.translate(self.translator, self.args.lang, dest)
            srt.wrap_lines()
            srt.save(f"{os.path.splitext(filepath)[0]}.{dest}.srt")
    ...
    def _transcribe(self, audio, speech_timestamps):
        ...
        if self.whisper_model is None:
            self.whisper_model = whisper.load_model("medium", self.args.device)
